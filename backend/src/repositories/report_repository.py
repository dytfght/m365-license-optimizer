...
class ReportRepository(BaseRepository[Report]):
    ...
    async def get_active_reports(
        ...
        query = (
            select(Report)
            .where(or_(Report.expires_at.is_(None), Report.expires_at > now))
            .order_by(Report.created_at.desc())
        )
    ...
    async def get_expired_reports(
        ...
        query = (
            select(Report)
            .where(and_(Report.expires_at.is_not(None), Report.expires_at <= now))
    ...
    async def get_reports_by_date_range(
        ...
        query = (
            select(Report)
            .where(and_(Report.created_at >= start_date, Report.created_at <= end_date))
            .order_by(Report.created_at.desc())
        )
