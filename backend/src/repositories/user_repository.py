...
class UserRepository(BaseRepository[User]):
    ...
    async def get_by_upn(self, upn: str, tenant_id: UUID) -> User | None:
        ...
        result = await self.session.execute(
            select(User).where(
                User.user_principal_name == upn, User.tenant_client_id == tenant_id
            )
        )
        return result.scalar_one_or_none()
    ...
    async def get_by_tenant(
        self, tenant_id: UUID, limit: int = 1000, offset: int = 0
    ) -> list[User]:
    ...
    async def sync_licenses(
        ...
        logger.debug(
            "user_licenses_synced", user_id=user_id, license_count=len(license_entities)
        )
