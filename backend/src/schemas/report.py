...
class ReportCreateRequest(BaseModel):
    ...
    analysis_id: UUID = Field(..., description="ID of the analysis to generate report for")
...
class ReportResponse(BaseModel):
    ...
    report_metadata: Dict[str, Any] = Field(default_factory=dict, description="Report metadata (KPIs, stats)")
    generated_by: str = Field(..., description="Email of user who generated the report")
    
    created_at: datetime = Field(..., description="Creation timestamp")
    expires_at: Optional[datetime] = Field(None, description="Expiration timestamp for cleanup")
...
class ReportMetadata(BaseModel):
    ...
    top_recommendations: List[Dict[str, Any]] = Field(
        ...
        description="Top 3 recommendations by savings"
    )
    
    # Charts data
    license_distribution: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="License distribution data for charts"
    )
...
class ReportFilter(BaseModel):
    ...
    report_type: Optional[str] = Field(None, description="Filter by report type (PDF/EXCEL)")
    date_from: Optional[datetime] = Field(None, description="Reports created after this date")
    date_to: Optional[datetime] = Field(None, description="Reports created before this date")
