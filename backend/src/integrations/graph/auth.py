...
class GraphAuthService:
    ...
    async def get_token(
        ...
        try:
            ...
            logger.error(
                "graph_auth_request_failed",
                tenant_id=tenant_id,
                error=str(e)
            )
            raise GraphAuthError(f"Network error during authentication: {str(e)}")
    
    def clear_cache(self, tenant_id: str = None, client_id: str = None):
        """
        Clear token cache.
        ...
        if tenant_id and client_id:
            ...
            logger.info("graph_token_cache_cleared", tenant_id=tenant_id, client_id=client_id)
        elif tenant_id:
            keys_to_remove = [k for k in self._token_cache.keys() if k.startswith(f"{tenant_id}:")]
            for key in keys_to_remove:
                self._token_cache.pop(key, None)
            logger.info("graph_token_cache_cleared_tenant", tenant_id=tenant_id, count=len(keys_to_remove))
        else:
