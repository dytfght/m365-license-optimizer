...
class AuthService:
    ...
    async def authenticate_user(self, email: str, password: str) -> dict[str, Any]:
        ...
        if not hasattr(user, 'password_hash') or not user.password_hash:
            logger.warning("authentication_failed", email=email, reason="no_password_hash")
            raise AuthenticationError("Invalid credentials")
        ...
        if not user.account_enabled:
            logger.warning("authentication_failed", email=email, reason="account_disabled")
    ...
    async def refresh_access_token(self, refresh_token: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not user:
                logger.warning("refresh_failed", user_id=user_id_str, reason="user_not_found")
                raise AuthenticationError("User not found")
            
            # Check if account is enabled
            if not user.account_enabled:
                logger.warning("refresh_failed", user_id=user_id_str, reason="account_disabled")
                raise AuthenticationError("Account is disabled")
