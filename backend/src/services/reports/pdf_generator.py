...
class PDFGenerator:
    ...
    def _create_kpi_section(self, data: Dict[str, Any]) -> Table:
        ...
        kpi_data = [
            [
                ...
                Paragraph(
                    f"{kpis.get('current_monthly_cost', 0):,.0f} €",
            ...
            [
                ...
                Paragraph(
                    f"{kpis.get('monthly_savings', 0):,.0f} €", self.styles["KPIValue"]
                ),
                Paragraph(
                    f"({kpis.get('savings_percentage', 0):.0f}%)",
            ...
            [
                ...
                Paragraph(
                    f"{kpis.get('annual_savings', 0):,.0f} €", self.styles["KPIValue"]
                ),
                "",
            ],
            [
    ...
    def _create_license_distribution_chart(self, data: Dict[str, Any]) -> Table:
        ...
        for i, item in enumerate(license_data[:6]):  # Max 6 items
            ...
            labels.append(
                f"{item.get('license_name', 'Unknown')} ({item.get('percentage', 0)}%)"
            )
    ...
    def _create_departments_table(self, data: Dict[str, Any]) -> Table:
        ...
        if not departments:
            return Table(
                [["Aucune donnée de département disponible"]], colWidths=[19 * cm]
            )
        ...
        dept_table = Table(
            table_data, colWidths=[5 * cm, 3 * cm, 3 * cm, 3 * cm, 5 * cm]
        )
    ...
    def _create_donut_chart_matplotlib(self, data: Dict[str, Any]) -> bytes:
        ...
        # Prepare data
        labels = [
            item.get("license_name", f"License {i}")
