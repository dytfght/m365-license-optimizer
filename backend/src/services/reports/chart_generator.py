...
class ChartGenerator:
    ...
    async def create_donut_chart(
        ...
        logger.info("donut_chart_created_successfully", size_bytes=len(img_buffer.getvalue()))
    ...
    async def create_bar_chart(
        ...
        for i, (x, y) in enumerate(zip(x_values, y_values)):
            if horizontal:
                ax.text(y + max(y_values) * 0.01, i, f'{y:,.0f}', 
                       va='center', fontsize=9, color=self.MICROSOFT_COLORS[0])
            else:
                ax.text(i, y + max(y_values) * 0.01, f'{y:,.0f}', 
                       ha='center', fontsize=9, color=self.MICROSOFT_COLORS[0])
        ...
        logger.info("bar_chart_created_successfully", size_bytes=len(img_buffer.getvalue()))
    ...
    async def create_savings_chart(
        ...
        sorted_depts = sorted(departments_data, key=lambda x: x.get("annual_savings", 0), reverse=True)
        ...
        bars = ax.barh(dept_names, savings, color=self.MICROSOFT_COLORS[0])
        
        # Add value labels
        for i, (dept, saving) in enumerate(zip(dept_names, savings)):
            ax.text(saving + max(savings) * 0.01, i, f'{saving:,.0f}€', 
                   va='center', fontsize=9, color=self.MICROSOFT_COLORS[0])
        ...
        ax.set_xlabel('Économies annuelles (€)', fontsize=10, color=self.MICROSOFT_COLORS[0])
        ...
        logger.info("savings_chart_created_successfully", size_bytes=len(img_buffer.getvalue()))
    ...
    async def create_usage_trend_chart(
        ...
        logger.info("usage_trend_chart_created_successfully", size_bytes=len(img_buffer.getvalue()))
    ...
    async def create_kpi_dashboard(
        ...
        ax1.text(0.5, 0.7, f"{kpis.get('monthly_savings', 0):,.0f}", 
                fontsize=36, fontweight='bold', color=self.MICROSOFT_COLORS[0],
                ha='center', va='center')
        ax1.text(0.5, 0.3, "Économie mensuelle (€)", 
                fontsize=12, color=self.MICROSOFT_COLORS[2],
                ha='center', va='center')
        ax1.set_xlim(0, 1)
        ax1.set_ylim(0, 1)
        ax1.axis('off')
        ...
        if license_dist:
            ...
            ax2.set_title("Répartition licences", fontsize=10, color=self.MICROSOFT_COLORS[0])
        ax2.axis('equal')
        ...
        if dept_data:
            ...
            bars = ax3.barh(depts, savings, color=self.MICROSOFT_COLORS[0])
            ax3.set_title("Économies par département", fontsize=10, color=self.MICROSOFT_COLORS[0])
        ...
        if activity_data:
            ...
            for i, (service, level) in enumerate(zip(services, levels)):
                color = self.MICROSOFT_COLORS[4] if level > 0.7 else self.MICROSOFT_COLORS[1] if level > 0.3 else self.MICROSOFT_COLORS[6]
                ax4.barh(i, level, color=color, height=0.8)
            
            ax4.set_yticks(range(len(services)))
            ax4.set_yticklabels(services)
            ax4.set_title("Niveaux d'activité", fontsize=10, color=self.MICROSOFT_COLORS[0])
            ax4.set_xlim(0, 1)
        ...
        if trend_data:
            ...
            ax5.plot(dates, savings, color=self.MICROSOFT_COLORS[0], linewidth=2, marker='o')
            ax5.set_title("Évolution des économies", fontsize=10, color=self.MICROSOFT_COLORS[0])
            ax5.grid(True, alpha=0.3)
        
        # KPI 6: Risk assessment (text)
        ax6 = fig.add_subplot(2, 3, 6)
        risk_level = kpis.get("risk_level", "Moyen")
        risk_color = {"Faible": self.MICROSOFT_COLORS[4], "Moyen": self.MICROSOFT_COLORS[1], "Élevé": self.MICROSOFT_COLORS[5]}
        
        ax6.text(0.5, 0.7, risk_level, fontsize=24, fontweight='bold',
                color=risk_color.get(risk_level, self.MICROSOFT_COLORS[0]),
                ha='center', va='center')
        ax6.text(0.5, 0.3, "Niveau de risque", fontsize=10,
                color=self.MICROSOFT_COLORS[2], ha='center', va='center')
        ax6.set_xlim(0, 1)
        ax6.set_ylim(0, 1)
        ax6.axis('off')
        
        # Global title
        fig.suptitle(title, fontsize=16, fontweight='bold', color=self.MICROSOFT_COLORS[0])
        ...
        logger.info("kpi_dashboard_created_successfully", size_bytes=len(img_buffer.getvalue()))
        
        return img_buffer.getvalue()

    def get_chart_colors(self, count: int) -> List[str]:
        """Get a list of Microsoft colors for charts"""
        return self.MICROSOFT_COLORS[:count] if count <= len(self.MICROSOFT_COLORS) else self.MICROSOFT_COLORS * (count // len(self.MICROSOFT_COLORS)) + self.MICROSOFT_COLORS[:count % len(self.MICROSOFT_COLORS)]