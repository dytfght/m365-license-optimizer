...
class ExcelGenerator:
    ...
    async def generate_detailed_excel(self, data: Dict[str, Any], language: str = "en") -> bytes:
    ...
    def _create_summary_sheet(self, wb: Workbook, data: Dict[str, Any], sheet_name: str, language: str):
        ...
        ws["D5"] = i18n_service.translate("report.license_distribution", language)
        ws["D5"].font = Font(
            name="Segoe UI", size=12, bold=True, color=self.COLORS["primary"]
        )
    ...
    def _create_detailed_sheet(self, wb: Workbook, data: Dict[str, Any], sheet_name: str, language: str):
        ...
        # Title
        ws["A1"] = i18n_service.translate("report.detailed_recommendations", language)
        ws["A1"].font = Font(
        ...
        for i, rec in enumerate(recommendations, start=4):
            ...
            ws.cell(
                row=i,
                column=14,
                value=i18n_service.translate("report.yes", language) if rec.get("office_desktop_required", False) else i18n_service.translate("report.no", language),
            )
    ...
    def _create_raw_data_sheet(self, wb: Workbook, data: Dict[str, Any], sheet_name: str, language: str):
        ...
        # Title
        ws["A1"] = i18n_service.translate("report.raw_data", language)
        ws["A1"].font = Font(
            name="Segoe UI", size=12, bold=True, color=self.COLORS["primary"]
        )

        # For testing, just add some sample data
        ws["A3"] = i18n_service.translate("report.data_type", language)
        ws["B3"] = i18n_service.translate("report.value", language)
        ws["C3"] = i18n_service.translate("report.date", language)
