...
from openpyxl.styles import (
    Alignment, Border, Font, PatternFill
)
...
class ExcelGenerator:
    ...
    def _create_detailed_sheet(self, wb: Workbook, data: Dict[str, Any]):
        ...
        for i, header in enumerate(headers, 1):
            ...
            cell.font = Font(name="Segoe UI", size=11, bold=True, color=self.COLORS['white'])
    ...
    def _create_raw_data_sheet(self, wb: Workbook, data: Dict[str, Any]):
        ...
        sample_data = [
            ["Total utilisateurs", data.get("kpis", {}).get("total_users", 0), datetime.now()],
            ["Économies mensuelles", data.get("kpis", {}).get("monthly_savings", 0), datetime.now()],
            ["Économies annuelles", data.get("kpis", {}).get("annual_savings", 0), datetime.now()],
        ]
