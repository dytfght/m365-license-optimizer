...
class ExcelGenerator:
    ...
    def setup_styles(self):
        ...
        self.header_style.font = Font(name="Segoe UI", size=12, bold=True, color=self.COLORS['white'])
        self.header_style.fill = PatternFill(start_color=self.COLORS['primary'], end_color=self.COLORS['primary'], fill_type="solid")
        ...
        self.kpi_value_style.font = Font(name="Segoe UI", size=14, bold=True, color=self.COLORS['primary'])
        self.kpi_value_style.alignment = Alignment(horizontal="center", vertical="center")
        ...
        self.red_fill = PatternFill(start_color="FFE8E8", end_color="FFE8E8", fill_type="solid")
        self.yellow_fill = PatternFill(start_color="FFF8E8", end_color="FFF8E8", fill_type="solid")
    ...
    def _create_summary_sheet(self, wb: Workbook, data: Dict[str, Any]):
        ...
        ws["A1"].font = Font(name="Segoe UI", size=16, bold=True, color=self.COLORS['primary'])
        ws.merge_cells("A1:H1")
        
        # Date
        ws["A3"] = f"Date de génération: {datetime.now().strftime('%d/%m/%Y à %H:%M')}"
        ws["A3"].font = Font(name="Segoe UI", size=10, color=self.COLORS['text_secondary'])
        
        # KPI Section (A5:B10)
        ws["A5"] = "INDICATEURS CLÉS"
        ws["A5"].font = Font(name="Segoe UI", size=12, bold=True, color=self.COLORS['primary'])
        ...
        ws["D5"].font = Font(name="Segoe UI", size=12, bold=True, color=self.COLORS['primary'])
        ...
        if not chart_df.empty:
            ...
            data_ref = Reference(ws, min_col=5, min_row=8, max_row=8+len(chart_df), max_col=5)
    ...
    def _create_detailed_sheet(self, wb: Workbook, data: Dict[str, Any]):
        ...
        ws["A1"].font = Font(name="Segoe UI", size=14, bold=True, color=self.COLORS['primary'])
        ws.merge_cells("A1:R1")
        
        # Headers
        headers = [
            "UserPrincipalName", "DisplayName", "Department", "JobTitle",
            "CurrentLicense", "CurrentMonthlyCost", "RecommendedLicense", "RecommendedMonthlyCost",
            "MonthlySavings", "AnnualSavings", "ExchangeUsageLevel", "OneDriveUsageGB",
            "TeamsActivity", "OfficeDesktopRequired", "LastActivityDate", "InactivityDays",
            "RecommendationStatus", "RecommendationReason"
        ]
        ...
        for i, rec in enumerate(recommendations, start=4):
            ...
            ws.cell(row=i, column=14, value="Oui" if rec.get("office_desktop_required", False) else "Non")
    ...
    def _add_data_validations(self, ws):
        ...
        usage_validation = DataValidation(
            type="list",
            formula1='"Faible,Moyen,Élevé,Aucun"',
            allow_blank=True
        )
        ...
        status_validation = DataValidation(
            type="list",
            formula1='"Proposé,Validé,Rejeté,Sensible"',
            allow_blank=True
        )
    ...
    def _create_raw_data_sheet(self, wb: Workbook, data: Dict[str, Any]):
        ...
        ws["A1"].font = Font(name="Segoe UI", size=12, bold=True, color=self.COLORS['primary'])
    ...
    def _prepare_excel_data(self, data: Dict[str, Any]) -> Dict[str, pd.DataFrame]:
        ...
        for rec in recommendations:
            detailed_data.append({
                ...
                "OfficeDesktopRequired": "Oui" if rec.get("office_desktop_required", False) else "Non",
