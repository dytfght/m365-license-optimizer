...
class ReportService:
    ...
    async def get_report_by_id(self, report_id: UUID) -> Optional[Report]:
        ...
        result = await self.session.execute(
            select(Report).where(
                Report.id == report_id,
                (Report.expires_at.is_(None)) | (Report.expires_at > datetime.now(timezone.utc))
            )
        )
    ...
    async def cleanup_expired_reports(self) -> int:
        ...
        for report in expired_reports:
            try:
                ...
                logger.info("expired_report_cleaned", 
                           report_id=str(report.id), 
                           file_path=str(file_path))
                
            except Exception as e:
                logger.error("failed_to_cleanup_report", 
                           report_id=str(report.id), 
                           error=str(e))
    ...
    async def _prepare_report_data(self, analysis: Analysis) -> Dict[str, Any]:
        ...
        # Top recommendations
        top_recommendations = self._prepare_top_recommendations(analysis.recommendations)
        ...
        tenant_name = "Tenant"  # Valeur par défaut, on pourrait chercher le tenant si nécessaire
        
        tenant_info = {
            "name": tenant_name,
            "period_start": analysis.analysis_date.strftime("%d/%m/%Y") if analysis.analysis_date else datetime.utcnow().strftime("%d/%m/%Y"),
            "period_end": (analysis.analysis_date + timedelta(days=28)).strftime("%d/%m/%Y") if analysis.analysis_date else datetime.utcnow().strftime("%d/%m/%Y"),
        }
    ...
    def _prepare_license_distribution(self, summary: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Prepare license distribution data for charts"""
        
        # This would come from the analysis breakdown
        # For now, return mock data structure
        return summary.get("license_breakdown", [
    ...
    def _prepare_top_recommendations(self, recommendations: List) -> List[Dict[str, Any]]:
        ...
        for rec in recommendations:
            ...
            recommendation_groups[rec_type]["annual_savings"] += (rec.savings_monthly * 12)
    ...
    def _prepare_departments_breakdown(self, recommendations: List) -> List[Dict[str, Any]]:
    ...
    async def _save_report_file(self, content: bytes, file_name: str, mime_type: str) -> str:
        ...
        try:
            ...
            logger.error("failed_to_save_report_file", 
                        file_path=str(file_path), 
                        error=str(e))
