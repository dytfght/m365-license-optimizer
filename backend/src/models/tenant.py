...
class TenantAppRegistration(Base, UUIDMixin, TimestampMixin):
    ...
    client_id: Mapped[str] = mapped_column(
        String(36), nullable=False, comment="Azure AD Application (client) ID"
    )
    client_secret_encrypted: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Encrypted client secret (use Fernet)"
    )
    ...
    authority_url: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="Azure AD authority URL"
    )
    ...
    tenant: Mapped["TenantClient"] = relationship(
        "TenantClient", back_populates="app_registration"
    )
