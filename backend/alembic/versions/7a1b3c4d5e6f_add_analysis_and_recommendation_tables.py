...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_analyses_status", "analyses", ["status"], unique=False, schema="optimizer"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            nullable=True,
            comment="Current SKU ID (null if no license)",
        ),
        sa.Column(
            "recommended_sku",
            sa.String(length=100),
            nullable=True,
            comment="Recommended SKU ID (null = remove license)",
        ),
        sa.Column(
            "savings_monthly",
