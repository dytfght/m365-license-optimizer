...
@pytest.mark.asyncio
async def test_reports_api_available(
    ...
    response = await client.post(
        f"/api/v1/reports/analyses/{fake_id}/pdf",
    ...
    response = await client.post(
        f"/api/v1/reports/analyses/{fake_id}/excel",
        headers=auth_headers
    )
    assert response.status_code in [404, 500]
    ...
    response = await client.get(
        f"/api/v1/reports/analyses/{fake_id}",
        headers=auth_headers
    )
    ...
    response = await client.get(
        f"/api/v1/reports/tenants/{fake_id}",
        headers=auth_headers
    )
    assert response.status_code in [200, 404, 500]
...
@pytest.mark.asyncio
async def test_reports_endpoint_structure(
    client: AsyncClient, auth_headers: dict
):
...
@pytest.mark.asyncio
async def test_reports_error_responses(
    client: AsyncClient, auth_headers: dict
):
...
@pytest.mark.asyncio
async def test_reports_endpoint_validation(
    client: AsyncClient, auth_headers: dict
):
    ...
    response = await client.get(
        f"/api/v1/reports/tenants/{fake_id}?limit=invalid",
        headers=auth_headers
    )
    ...
    response = await client.get(
        f"/api/v1/reports/tenants/{fake_id}?limit=10&offset=5",
        headers=auth_headers
    )
    ...
    response = await client.get(
        f"/api/v1/reports/tenants/{fake_id}?report_type=PDF",
        headers=auth_headers
    )
